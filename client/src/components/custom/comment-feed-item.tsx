...
export function CommentFeedItem({ comment, currentUser, onUpdate }: CommentFeedItemProps) {
  const [isEditing, setIsEditing] = useState(false)
  const [editContent, setEditContent] = useState(comment.content)
  const [isSubmitting, setIsSubmitting] = useState(false)

  const isOwner = currentUser?.userId === comment.author?.id
      ...
      <div className="flex items-start space-x-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center space-x-2 mb-1">
            <span className="font-semibold text-foreground">{comment.author?.username || 'Unknown User'}</span>
